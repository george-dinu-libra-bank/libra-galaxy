...
class _ClientRol:
    ...
    def execute(self):
        class R:
            data = [{"role": "admin"}] if self._rol == "admin" else []
...
def ca_admin():
    ...
    app.dependency_overrides[get_admin_supabase] = lambda: _ClientRol("admin")
    yield
...
def test_un_utilizator_obisnuit_primeste_403(depozit_fals) -> None:
    app.dependency_overrides[get_current_user] = lambda: ADMIN
    app.dependency_overrides[get_admin_supabase] = lambda: _ClientRol(None)
    try:
