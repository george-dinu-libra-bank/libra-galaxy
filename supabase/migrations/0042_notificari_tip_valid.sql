-- =============================================================================
-- 0042 — Notificarile deciziilor foloseau un `tip` care nu exista
--
-- CAUZA, gasita tot ruland fluxul cap-coada, nu citind codul:
--
--   ERROR: 23514 new row for relation "notificari" violates check constraint
--          "notificari_tip_check"
--   DETAIL: ... Contul "Vacanta" a fost inchis. Am mutat 86898002826.81 RON in...
--   CONTEXT: insert into public.notificari ... PL/pgSQL function
--            public.inchide_cont_bancar(uuid,uuid,uuid) line 133
--
-- `notificari.tip` accepta doar patru valori: 'info', 'atentionare', 'blocare',
-- 'deblocare'. RPC-urile de decizie scriau 'succes' si 'avertisment' — cuvinte
-- care suna bine, dar pe care baza nu le cunoaste.
--
-- Efectul e mai rau decat pare, fiindca insertul e ULTIMUL pas: pana acolo banii
-- fusesera deja mutati si contul inchis, iar exceptia da inapoi toata tranzactia.
-- Nu ramanea o stare gresita — dar operatiunea esua intreaga, de fiecare data, si
-- din a doua cauza la rand.
--
-- Traducerea in vocabularul existent, nu invers (adica NU largirea constrangerii):
-- `frontend/src/lib/data/notificari.ts` tipeaza `tip` ca uniune inchisa peste fix
-- cele patru valori, deci o a cincea ar fi trecut de baza si ar fi cazut la
-- typecheck sau, mai rau, ar fi ajuns nestilizata pe ecran.
--
--   aprobare / reusita  -> 'info'
--   respingere          -> 'atentionare'
--
-- Restul corpului functiilor e identic cu 0041.
-- =============================================================================
...
begin
  ...
  values (v_cerere.id_utilizator, v_titlu, v_mesaj,
          case when p_aproba then 'info' else 'atentionare' end);
...
begin
  ...
  insert into public.notificari (id_utilizator, titlu, mesaj, tip)
  values (v_cerere.id_utilizator, 'Contul a fost inchis', v_mesaj, 'info');
